...
def video_player(video, user):
    ...
    path = abs_path + '/video/'
    exist = False
    # if os.path.isfile(abs_path + '/capture/' + 'User' + str(user) + '_Video' + str(video) + '.avi'):
    if os.path.isfile(abs_path + '/capture/Video' + str(video) + '.avi'):
    ...
    save_out = cv2.VideoWriter((abs_path + '/capture/Video' + str(video) + '.avi'),
                               fourcc, 30.0, (cam_width, cam_height))
